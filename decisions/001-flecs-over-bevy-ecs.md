...
**Date:** 2026-06-12 · **Status:** Accepted — **CONFIRMED at M0 gate review 2026-06-13** (`spikes/flecs`). Native-only for wasm; browser query path resolved by [ADR-006](006-browser-query-backend.md). M1 *integration* go/no-go is a normal roadmap milestone (§7), not an open decision. · **Supersedes:** —
...
> **Web-target caveat (2026-06-13, from spike ③ `spikes/wasm`):** `flecs_ecs` 0.2.2 **does not compile to `wasm32-unknown-unknown`** — its C core needs clang + a wasm libc/sysroot the bare target lacks. Desktop is unaffected (this ADR is desktop-first), but the **browser** lite-editor (ADR-004 funnel) cannot run Flecs client-side as-is. This is a new revisit dimension beyond safety/perf/maintenance. **Resolved 2026-06-13 by [ADR-006](006-browser-query-backend.md):** the browser does not run Flecs — the query-API wrapper gets a second, pure-Rust backend over the Loro projection (Phase 2); Flecs is native-only. `loro` builds for wasm32. See [ADR-003](003-desktop-first-tauri-exit-gate.md) and `spikes/wasm/CONSTRAINTS.md`.
