...
| Binding by intent | **reveal engine live-wired (M3.1, `editor-shell/src/{reveal,capscene}.rs` + `src-tauri`)**: on select, run the M1.5 compat query on the engine's world → rank compatible targets (proximity · affinity · recency, deterministic offline — no LLM) + explain every "no" (`why_not`, O(1)/target); click a candidate → one-transaction bind (Loro edge + ECS `BindsTo` pair, undoable). Surfaced in the running shell ("Bind by intent" panel; `reveal_targets`/`bind_target`). **Headless north-star test #1 passes through the real engine**; live per-click reveal **p99 1.523 ms @5k** (release), bare reveal p99 1.107 ms. **Pending:** live human acceptance + dogfood verdict; live reload-persistence (the capability-rebuild carry-forward). | [011](decisions/011-intent-ranking.md) |
...
- ~~Real-scene render cost at ≥5k entities~~ → **resolved — render gate PASSED (M2.2, 2026-06-14)** ([ADR-003](decisions/003-desktop-first-tauri-exit-gate.md) status, `spikes/render-scene/RESULTS.md`): the M1.4 stress scene (5k + 20k, instanced cubes + per-entity gizmos + grid) renders well inside budget via instancing + GPU frustum culling (compute → indirect) + a render bundle. Native GPU p99 0.60 ms @5k / 0.88–0.95 ms @20k (~14×/17× headroom); browser (Chrome/Edge, Dawn) GPU p99 1.34 ms @5k / 3.26 ms @20k. Draw calls constant at 3 as entities go 4× (instancing engages); largest storage buffer 0.61 MB ≪ 128 MB; the no-multi-draw-indirect / `first_instance=0` path runs native + both browsers. **Gap:** Firefox 141 not run (not installed) — low risk.
