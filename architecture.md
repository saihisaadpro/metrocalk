...
| AI layer | **patch contract real (M6, `editor-shell/src/ai.rs`)**: every AI / generation scene mutation enters as a **schema-validated transactional patch** (`apply_ai_patch` — allow-listed ops checked against the registry schema + engine state: entity exists · known component · known field · strict value type), applied through the **one commit pipeline** (undoable) on success or **rejected-as-UX** (all-or-nothing) on failure. No raw/unvalidated LLM mutation path. Generation's stream-in rides this **live**; the AI-**edit** sibling ("make it rustier") is the same generic `apply_ai_patch` — built + unit-tested, with its **live editor command/UI the next increment** of this seam (`MeterAction::Edit` not yet on a live path; prompt 25 permitted documenting it as the next increment). **MCP server** itself = a seam. | [017](decisions/017-generation-tier-and-ai-patch-contract.md) · plan §2 |
...
| Asset generation + marketplace | **index tier real (M5, `core/src/marketplace.rs`)**: a `MarketplaceIndex` trait (a remote impl slots in unchanged) with a checked-in `LocalCatalog` of **pre-componentized** entries (namespaced caps + a prompt-23 asset handle + an inert price); the resolver's `resolve` is now tiered for real — local short-circuits offline, marketplace is the **2nd** tier, generate the seam. Choosing an entry applies a working object (caps + mesh) as one undoable, replay-persisted tx with the M3.1 attach. **M6 generation tier real (`editor-shell/src/generate.rs`):** a no-anywhere-match offers an **opt-in** Generate (tier 3, metered last resort) → a **grey placeholder** drops in instantly (one undoable tx) → the `MeshGenerator` provider (deterministic **fake**; real = a documented seam) runs off the hot path → the real mesh **streams in** through the prompt-23 importer as a validated AI patch (same id, targeted delta), undoable + replay-persisted; offline ⇒ honest seam. **Token economy + payout + the real text-to-3D provider stay seamed** — no money moves; the placeholder commit is p99 ~1 ms. | [017](decisions/017-generation-tier-and-ai-patch-contract.md) · [015](decisions/015-capability-identity-and-marketplace-index.md) · [004](decisions/004-free-engine-token-economy.md) |
