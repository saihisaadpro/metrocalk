...
| Binding by intent | **reveal engine real, headless (M3.1, `editor-shell/src/reveal.rs`)**: on select, run the M1.5 compat query → rank compatible targets (proximity · affinity · recency, deterministic offline — no LLM) + explain every "no" (`why_not`, O(1)/target). p50 0.706 ms / p99 1.107 ms @5k (release). One-click bind = the M2.6 `apply_edit` (undoable). **Live surfacing + north-star test 1 acceptance + dogfood verdict pending** (gated on M2 interactive close). | [011](decisions/011-intent-ranking.md) |
