...
| Persistence | **live (M2, `persist.rs`)**: deterministic re-seed (fixed seed → identical `EntityId`s) + replay an append-only `EditTx`/bind/undo log on launch, then `clear_history` (restored scene non-undoable). Deliberately **not** Loro merge-on-start (merge drops the ECS capability pairs the reveal needs). A bind survives close→reopen (test-1 box 5). | [002](decisions/002-loro-over-custom-wal.md) |
