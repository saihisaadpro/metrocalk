...
## Next (milestone M3.2 — in progress)
- **M3.2 — describe / type-to-create** (the second half of the north-star). **Started:** the local resolver landed (`core/src/resolve.rs`, ADR-012) — offline describe→component-kind with real capabilities, marketplace/generate seamed, wasm-safe; 4 tests green. **Remaining:** describe-input → `EditTx`; pre-componentized instantiate (undoable, persisted); one-click attach (reuse M3.1 reveal); offline+latency proof; wasm run-proof; the headless e2e (describe→componentize→attach→undo→reload); test #2 boxes honest. See `progress/M3.md`.
