...
- **Not yet closed (no fabrication):** (a) **the human gate** — test #1 run *in the window* + the **dogfood verdict** ("does it feel like the categorical win" — the judgment IS the finding); (b) **"survives reload" *live*** — proven headless, but the shell has no persistence layer yet (entangled with the capability-rebuild carry-forward below); (c) M2's **zero-IPC-drag** (native input routing) + the **six residuals** (DPI/min-spec/flicker = human/hardware; Channel-e2e/snapshot/browser-store = WebView2-CDP driver). **M2 is integrated + all measurable gates green; NOT declared complete** (live human acceptance pending). See `progress/M3.md`.
