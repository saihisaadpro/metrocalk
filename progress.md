...
## Now
- **M2 complete (code + all measurable gates); north-star #1 real.** On `master`/`origin/main`: the live `/editor-shell` (Tauri 2, ADR-008 single-window composite over the real `/core`, no MockCore) runs north-star test #1 **in the window** — click a HealthBar → ranked-by-proximity compatible + every-no-explained → one-click undoable bind (**4/5 boxes human-confirmed 2026-06-15**) — and now **survives reload** (box 5: deterministic-seed + replay-log, `persist.rs`; headless-proven). Camera **orbit/zoom + the viewport hot path are zero-per-frame-IPC** (inv. 4; only `drag_start`/`drag_end` cross, instrumented by `render::IPC_CALLS`). Left-click pick is DPI-safe (normalized cursor).
