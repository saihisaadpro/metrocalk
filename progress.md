...
## Now
- **M2 complete; north-star #1 real and MACHINE-VERIFIED live.** A real-`.exe` E2E harness (WebdriverIO + tauri-driver, `editor-shell/e2e/`) drives the packaged app's WebView2 — including the transparent viewport `<div>` → native pick — and **north-star test #1 passes 7/7 live**: launch + connect (5000) · requirer → ranked reveal · one-click bind → tracking · single-step undo · **viewport pick** · field edit. This also caught + fixed a real bug: the viewport pick was coupled to the render loop's frame cadence (request→poll), which stalled when the window wasn't presenting → "picked nothing here"; now computed **synchronously** in the `viewport_pick` command (pure projection, no race). On `master`/`origin/main`: the live `/editor-shell` (Tauri 2, ADR-008 composite over the real `/core`, no MockCore) — click a HealthBar → ranked-by-proximity compatible + every-no-explained → one-click undoable bind; **survives reload** (box 5: deterministic-seed + replay-log); camera **orbit/zoom + viewport hot path zero-per-frame-IPC** (inv. 4).
...
- **Handed off (human/hardware/driver — instrumented, not fabricated, not blocking):** the **dogfood verdict** (does it *feel* like the categorical win — the prize); the 30-s live close→reopen + drag-feel spot-checks; DPI 100↔200, ≥60 s flicker, full min-spec sweep; Channel-e2e re-confirm; real-browser store-apply. See `progress/M3.md`.

## Next (milestone M2 → M3.2)
- **M3.2 — describe / type-to-create** (the second half of the north-star): type "rusty medieval sword" → local → marketplace → generate; arrives pre-componentized; one-click attach. Author from the M2/M3.1 evidence.
- **Follow-ups (non-blocking):** incremental undo delta (replace `project_full`-on-undo, the ~70 ms hitch at 5k); feed **recency** the edit log (plumbed but inert live); the capability-rebuild carry-forward (so a future Loro merge/reload keeps capabilities).
