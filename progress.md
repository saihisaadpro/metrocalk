...
## Now
- **M2 build — consolidated to `master`; M2.6 convergence started.** All five M2 lanes (M2.1–M2.5) + M1.6 are merged and **on `master`**, pushed to `origin/main` (`ed1b9c0`), workspace builds + tests green, both CI grep gates green, ADRs 001–010 present. **M2.6 spine landed:** the `editor-shell` crate's real-core edit bridge — `EditTx` → `Engine::commit` → `ProjectionDelta` (confirm / reject-with-reason, undoable), **no MockCore**, 4 tests green through `Engine<FlecsWorld>`. **Remaining for M2 close (M2.6):** the Tauri `/editor-shell` app (transparent WebView2 over the wgpu viewport per ADR-008, M2.2 renderer, Rust picking, real Tauri Channel + Loro→projection seam), the live round-trip + zero-IPC-drag instrumentation, wasm parity, the six deferred residuals, and the **measured M2 go/no-go**. **M2 is NOT declared complete — the gate is unmeasured.**
