// ADR-195 — "keep the subject framed", proven on the packaged `.exe` by comparing CAMERAS.
//
// The claim cannot be passed by relabelling anything, and it has two halves that must BOTH hold: when
// the subject walks, the aim moves by exactly what the subject did and the eye does not move at all.
// A solver that re-placed the camera would satisfy the first and fail the second, and a feature that
// stored a flag and changed no pixels would fail both — while passing every DOM assertion there is.
//
// The negative control is in the same file and is what makes the rest evidence: the same walk, with
// the head locked off, must still film the floor the subject used to stand on.
//
...
//
// Run: node "node_modules\@wdio\cli\bin\wdio.js" run wdio.keepframed.conf.js
...
// this spec spawned, which is the only way "the camera is inside something" can be attributed.
...
export const config = {
  ...
  specs: ["./specs-keepframed/keep-framed.e2e.js"],
  maxInstances: 1,
