// LIVE M11.3 shadow quality-profile check: render a small shadow-casting scene in the packaged .exe and
// capture the composited window, so MTK_SHADOW_QUALITY=high (shadows on the ground) vs =off (no shadows)
// can be compared. MTK_SHADOW_QUALITY propagates from this process's env → tauri-driver → the app (same as
// MTK_SCENE_N). A few seeded cubes on the ground plane are the casters.
//
// Run (LOCAL): set MTK_SHADOW_QUALITY=high|off & set MTK_SHOT_DIR=<out> & set MTK_SHOT_PS1=<capture.ps1>
//   node "node_modules\@wdio\cli\bin\wdio.js" run wdio.shadowtest.conf.js
...
// A few casters on the ground plane (not the 5000-cube wall).
