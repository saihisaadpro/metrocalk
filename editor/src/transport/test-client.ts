...
export function fakeClient(over: Partial<EditorClient> = {}): EditorClient {
  return {
    ...
    cinemaStandAtShot: vi.fn((_id: string, index: number, progress?: number) => Promise.resolve({ moved: true, stood: [6, 3, 8] as [number, number, number], lookAt: [0, 0.5, 0] as [number, number, number], fovDeg: 40, offBy: 0, progress: progress ?? 0, placed: false, steps: 0, acceptable: false, hidden: 0.78, moving: false, travel: 0, worst: 0, path: [], message: `Standing where shot ${index + 1} films from — this is the best placement the engine found.`, reason: null })),
    ...
    cinemaSetShotTracking: vi.fn((id: string, index: number, track: boolean) => Promise.resolve({ entity: id, shots: 1, seconds: 2.5, mood: "normal" as const, delivery: "viewport" as const, render: DEFAULT_RENDER_SETTINGS, reads: [track ? "a placed shot of Crate, pushing in, keeping it framed — 2.5s" : "a placed shot of Crate, pushing in — 2.5s"], rows: [{ ...HERO_ROW, index, camera: { eye: [6, 3, 8] as [number, number, number], lookAt: [0, 0.5, 0] as [number, number, number], fovDeg: 55, track: track ? ([0, 0.5, 0] as [number, number, number]) : null }, reads: track ? "a placed shot of Crate, pushing in, keeping it framed — 2.5s" : "a placed shot of Crate, pushing in — 2.5s" }], problems: [], message: track ? `Shot ${index + 1} keeps Crate framed — the camera stays where it is and only the aim follows` : `Shot ${index + 1} is locked off — it films exactly the frame you placed`, reason: null })),
