# Progress log — M0 (foundation, spikes, gate review)

Archived from `progress.md` at the M0→M1 boundary (2026-06-13). Append-only, newest first; one
entry per working session. Live Now/Next state stays in `progress.md`; this is the detailed
historical record for M0 and the pre-M0 setup. ADRs (the durable *why*) live in `decisions/`.
